...
import org.jbpm.gpd.renderer.TransitionView;
...
public class DefaultGraphModelFileFormatXML implements GraphModelFileFormat {
	 ...
	 * 
	 * @see org.jgraph.pad.GraphModelFileFormat#write(String, Hashtable,
	 *      GPGraph, GraphModel)
	 */
	...
	public GraphModel read(URL file, Hashtable properties, GpdGraph gpGraph)
		...
		try {
			...
			if (length == 4 && new String(ident).toLowerCase().equals("<?xm"))
				newFileFormat = true;
	...
	public void read(InputStream in, JGraph graph) throws Exception {
		...
		while (it.hasNext()) {
			...
			if (attr != null) {
				...
				// EFLOW
				try {
					DefaultGraphCell vertex = (DefaultGpdCell) att.getCell();
					vertex.setAttributes((AttributeMap) attr);
					...
					if (vertex instanceof StartCell)
						...
						view = new TransitionView(vertex);

					
					
					// graph.getGraphLayoutCache().insert(new Object[] { vertex});
					
					//if comentado devido a novas alterações
					// if(!(vertex instanceof ActivityCell)) {
					// graph.getGraphLayoutCache().insertViews(new
					// CellView[]{view});
					graph.getGraphLayoutCache().putMapping(att.getCell(), view);
					/*
					 * } else { ActivityCell activity = (ActivityCell) vertex;
					 * AttributeMap map = activity.getAttributes();
					 * GraphConstants.setBackground(map, new
					 * Color(activity.getModel().getRGBBackColor()));
					 * //JOptionPane.showMessageDialog(null,
					 * activity.getModel().getBackColor()); }
					 */
				} catch (Exception e) {
				}
			}
	...
	public static void applySettings(Map s, JGraph graph) {
		...
		/*
		 * tmp = s.get("selectNewCells"); if (tmp != null)
	 ...
	 * @param node
	 */
	private void readJBPMData(DefaultGraphCell cell, Node node)
			throws MarshalException, ValidationException {
		
		if (cell instanceof SubProcessCell) {
			NodeList nodeList = ((Element) node)
					.getElementsByTagName("sub-process-vO");
			SubProcessCell aCell = (SubProcessCell) cell;
			SubProcessVO aVO = (SubProcessVO) Unmarshaller.unmarshal(
					SubProcessVO.class, nodeList.item(0));
			if(aVO!=null) {
				aCell.setModel(aVO);
			}
		} else if (cell instanceof ActivityCell) {
			NodeList nodeList = ((Element) node)
					.getElementsByTagName("activity-state-vO");
			ActivityCell aCell = (ActivityCell) cell;
			ActivityStateVO aVO = (ActivityStateVO) Unmarshaller.unmarshal(
					ActivityStateVO.class, nodeList.item(0));
			aCell.setModel(aVO);
		} else if (cell instanceof DecisionCell) {
			NodeList nodeList = ((Element) node)
					.getElementsByTagName("decision-vO");
			DecisionCell aCell = (DecisionCell) cell;
			aCell.setModel((DecisionVO) Unmarshaller.unmarshal(
					DecisionVO.class, nodeList.item(0)));
		} else if (cell instanceof EndCell) {
			NodeList nodeList = ((Element) node).getElementsByTagName("end-vO");
			EndCell aCell = (EndCell) cell;
			aCell.setModel((EndVO) Unmarshaller.unmarshal(EndVO.class, nodeList
					.item(0)));
		} else if (cell instanceof StartCell) {
			NodeList nodeList = ((Element) node)
					.getElementsByTagName("start-state-vO");
			StartCell aCell = (StartCell) cell;
			aCell.setModel((StartStateVO) Unmarshaller.unmarshal(
					StartStateVO.class, nodeList.item(0)));
		} else if (cell instanceof ForkCell) {
			NodeList nodeList = ((Element) node)
					.getElementsByTagName("fork-join-vO");
			ForkCell aCell = (ForkCell) cell;
			aCell.setModel((ForkJoinVO) Unmarshaller.unmarshal(
					ForkJoinVO.class, nodeList.item(0)));
			...
			NodeList nodeList = ((Element) node)
					.getElementsByTagName("transition-vO");
			Transition aCell = (Transition) cell;
			aCell.setModel((TransitionVO) Unmarshaller.unmarshal(
					TransitionVO.class, nodeList.item(0)));
		}
	}
	...
	public String toString(JGraph graph) throws MarshalException,
			ValidationException {
		userObjectMap.clear();
		cellMap.clear();
		attrs.clear();
	 ...
	 * @return
	 */
	private String writeJBPMData(Object cell, String indent)
			throws MarshalException, ValidationException {
		...
		if (propertydata.toString().length() > 0) {
			returnValue += indent + "    "
					+ propertydata.toString().substring(39) + "\n";
		}
	...
	public static String[] knownKeys = new String[] { GraphConstants.ABSOLUTEX,
			GraphConstants.AUTOSIZE, GraphConstants.BACKGROUND,
			...
			GraphConstants.MOVEABLE, GraphConstants.OFFSET,
			GraphConstants.OPAQUE, GraphConstants.POINTS,
			GraphConstants.ROUTING, GraphConstants.SIZE,
			GraphConstants.SIZEABLE, GraphConstants.VALUE };

	public static Class[] keyTypes = new Class[] { Boolean.class,
			Boolean.class, Color.class, Boolean.class, Integer.class,
			Boolean.class, Border.class, Color.class, Rectangle2D.class,
			Boolean.class, float[].class, Boolean.class, Boolean.class,
			Boolean.class, Integer.class, Font.class, Color.class,
			Integer.class, Integer.class, Icon.class, Point.class,
			Integer.class, Color.class, Integer.class, Integer.class,
			Float.class, Boolean.class, Point.class, Boolean.class, List.class,
			Edge.Routing.class, Dimension.class, Boolean.class, Object.class };
	...
	public static String encodeValue(Object value) {
		...
		if (value instanceof Rectangle2D) {
			...
			ret = (int) point.getX() + "," + (int) point.getY();
	...
	public static Map decodeMap(Node node, boolean useKnownKeys,
	...
	// adicionado novos tipos de celulas
	public static DefaultGraphCell createCell(String type, Object userObject) {
		if (type.equals("activityDecision")) {
			return new ActivityDecisionCell(userObject);
		} else if (type.equals("subProcessDecision")) {
	...
	public static String getType(Object cell) {
		if (cell instanceof ActivityDecisionCell)
			return "activityDecision";
		else if (cell instanceof SubProcessCell)
			return "subProcessDecision";
		else if (cell instanceof ActivityCell)
			return "activity";
		else if (cell instanceof DecisionCell)
			return "decision";
