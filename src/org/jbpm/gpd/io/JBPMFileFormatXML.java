...
public class JBPMFileFormatXML extends AbstractJBPMFileFormatXML {
	...
	private List getActionClasses(List cellList, List result) {
		Iterator startIt=cellList.iterator();
                while(startIt.hasNext()){
                        ...
                        //if(action.getOnException()!=null) result.add(action.getOnException());
                }
                return result;
        }

        private List getTransitionClasses(List transitionList, List result) throws InvalidModelException {
...
//System.out.println("\nCells count: "+max);
 	...
 	private List getClassesBlock(List transitionList, List result) throws InvalidModelException {
                Iterator it = transitionList.iterator();
		while(it.hasNext()){
			Transition transition=(Transition)it.next();
                        Object cellObject= getGraphModel().getParent(getGraphModel().getTarget(transition));
                        if (!exportedCellClassList.contains(cellObject)){
                                ...
                                if (cellObject instanceof ActivityCell){
                                        ActivityStateVO activityModel = ((ActivityCell)cellObject).getModel();
                                        ...
                                        DecisionVO decisionModel = ((DecisionCell)cellObject).getModel();
                                        ...
                                        //OLD JoinCell joinCell=(JoinCell)joinCellClassStack.getFirst();
                                        ...
                                        //OLD if (!joinCellClassStack.contains(cellObject)) joinCellClassStack.add(cellObject);
                                }
                                if (!transit.isEmpty()) {
	...
	public String toString(JGraph graph) throws InvalidModelException {
		...
		out.append("  \"jbpm/processdefinition_1_0\"\n");
		out.append("  \"http://jbpm.org/dtd/processdefinition_1_0.dtd\">\n");
		...
		writeActionInfo(startCell.getModel().getProcessDefinition().getActionList());
	 ...
	 * @param model
	 */
	private void writeObjectsByTransitions(List transitionList) throws InvalidModelException {
		Iterator it = transitionList.iterator();
		while(it.hasNext()){
			Transition transition=(Transition)it.next();
	 ...
	 * @param model
	 */
	private void writeObjectByTransition(Transition transition) throws InvalidModelException {
		Object cellObject= getGraphModel().getParent(getGraphModel().getTarget(transition));
	 ...
	 * @param cell
	 */
	private void writeConcurrentBlock(ForkCell cell) throws InvalidModelException {
		...
		if (transition.isEmpty()){
			throw new InvalidModelException("fork "+model.getName()+" has no transitions!");
		}
		...
		if (transition.isEmpty()){
			throw new InvalidModelException("join "+model.getName()+" has no transitions!");
		}
		writeTransitonInfo(transition);
		out.append("    </join>\n");
		//cache.append(out);
	...
	private void writeDecision(DecisionCell cell) throws InvalidModelException {
		...
		out.append("  <decision name=\""+model.getName()+"\" handler=\""+model.getHandler()+"\">\n");
		...
		List transition=findTransitionBySource(cell);
		if (transition.isEmpty()){
			throw new InvalidModelException("Decision "+model.getName()+" has no transitions!");
		}
		writeTransitonInfo(transition);
		out.append("  </decision>\n");
		writeObjectsByTransitions(transition);
	 ...
	 * @param cell
	 */
	private void writeActivityState(ActivityCell cell) throws InvalidModelException {
		ActivityStateVO activityModel = cell.getModel();
		out.append("  <activity-state name=\""+activityModel.getName()+"\">\n");
		out.append("    <description>"+activityModel.getDescription()+"</description>\n");
		AssignmentVO assignment = activityModel.getAssignment();
		if (assignment!=null){
		...
		List transition=findTransitionBySource(cell);
		if (transition.isEmpty()){
			throw new InvalidModelException("ActivityState "+activityModel.getName()+" has no transitions!");
		}
		writeTransitonInfo(transition);
	...
	private StartCell writeStartEndModelInfo()
		...
		out.append("  <responsible>"+processDefiniton.getResponsible()+"</responsible>\n\n");
		out.append("  <!-- ====================== -->\n");
		out.append("  <!-- == START & ENDSTATE == -->\n");
		out.append("  <!-- ====================== -->\n");
		out.append("  <start-state name=\""+startModel.getName()+"\">\n");
		out.append("    <description>"+startModel.getDescription()+"</description>\n");
	...
	private void writeFieldInfo(List list) {
		...
		while(it.hasNext()){
			...
			out.append("    <field attribute=\""+field.getAttribute()+"\" access=\""+field.getAccess().trim()+"\" />\n");
		}
		
	}

	private void writeTransitonInfo(List transitionList) throws InvalidModelException {
		Iterator it = transitionList.iterator();
		while(it.hasNext()){
			Transition transition=(Transition)it.next();
			Object targetObject= getGraphModel().getParent(getGraphModel().getTarget(transition));
			String targetName=getCellName(targetObject);
			if (targetName==null){
				throw new InvalidModelException("transition need a target of type activitystate, decision, end, join or fork!");
			}

			TransitionVO transitionVo=transition.getModel();
			if (transitionVo.getName()==null || transitionVo.getName().length()==0){
				out.append("    <transition to=\""+targetName+"\">\n");
			}else {
				out.append("    <transition name=\""+transitionVo.getName()+"\" to=\""+targetName+"\">\n");
			}
	...
	private void writeActionInfo(List actionList) {
		...
		while(actionIt.hasNext()){
			...
			out.append("      <action event=\""+action.getEvent()+"\" handler=\""+action.getHandler()+"\" on-exception=\""+action.getOnException()+"\">\n");
	...
	private void writeVariableDef(ProcessDefinition processDefiniton) {
		...
		while (it.hasNext()){
			...
			if (attribute.getSerializer()!=null && attribute.getSerializer().length()>0){
				out.append(" serializer=\""+attribute.getSerializer()+"\" ");
			}
			if (attribute.getInitialValue()!=null && attribute.getInitialValue().length()>0){
	...
	private void writeFieldDef(ProcessDefinition processDefiniton) {
            ...
            for ( int i=0 ; i<max ; i++ )
            {
                ...
                {
                    Iterator it = ((ActivityCell)cell).getModel().getFieldList().iterator();
                    while (it.hasNext()){
			FieldVO field = (FieldVO)it.next();
			out.append("  <attribute name=\""+field.getName()+"\" ");
			if (field.getType()!=null && attribute.getType().length()>0){
				out.append(" type=\""+attribute.getType()+"\" ");
			}
			if (attribute.getSerializer()!=null && attribute.getSerializer().length()>0){
				out.append(" serializer=\""+attribute.getSerializer()+"\" ");
	 ...
	 * @param doc
	 */
	private void createDecision(GpdGraph gpGraph, Document doc) throws JDOMException {
	 ...
	 * @param doc
	 */
	private void createTransitions(GpdGraph gpGraph, Document doc) throws JDOMException, InvalidModelException {
		XPath transitionPath = XPath.newInstance("/process-definition//transition");
		...
		while (it.hasNext()){
			...
			String from=((Element)transition.getParent()).getAttribute("name").getValue();
	 ...
	 * @param to
	 */
	private void createTransition(String from, String to, Element transition) throws InvalidModelException {
		DefaultGpdCell fromCell = getCellByName(from);
		DefaultGpdCell toCell = getCellByName(to);
		((GpdMarqueeHandler)graph.getMarqueeHandler()).connect((Port)fromCell.getChildAt(0),(Port)toCell.getChildAt(0));
	 ...
	 * @param doc
	 */
	private void createForkJoin(GpdGraph gpGraph, Document doc) throws JDOMException {
	 ...
	 * @param doc
	 */
	private void createStartEnd(GpdGraph gpGraph, Document doc) throws JDOMException {
		XPath startstatePath = XPath.newInstance("/process-definition/start-state");
	 ...
	 * @param doc
	 */
	private void createProcessDefinition(ProcessDefinition definition, Document doc) throws JDOMException {
	...
	private void createActivityState(GpdGraph gpGraph, Document doc)
		...
		XPath activityPath = XPath.newInstance("/process-definition//activity-state");
