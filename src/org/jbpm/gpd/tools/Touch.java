		...
		for (int i = 0; i < vertices.length; i++) {
			...
			if (view != null) {
				...
				if (GraphConstants.isMoveable(view.getAllAttributes())
					...
					bounds.setRect(Math.max(0, (int) position.x - bounds.getWidth()/2), bounds.getY(), bounds.getWidth(), bounds.getHeight());
					bounds.setRect(bounds.getX(), Math.max(0, (int) position.y - bounds.getHeight()/2), bounds.getWidth(), bounds.getHeight());
					repaintNeeded = true;
